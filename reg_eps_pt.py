import scipy.io
from common.BNN_Sequential import BNN_Sequential
import sys
import pickle
import numpy as np 
import pdb
...
new_y = []
for episode in out:
    x = episode[:, :state_dim+action_dim]
    y = episode[:, state_dim+action_dim:] - episode[:, :state_dim]
    # new_ep = np.concatenate([episode[:, :state_dim+action_dim], y], axis=1)
    # new_eps.append(new_ep)
    new_x.append(x)
    new_y.append(y)
# DATA = np.concatenate(new_eps)
# x_data = np.concatenate(new_x)
# y_data = np.concatenate(new_y)
# new_state_dim = 6
# # pdb.set_trace()
